...
import { getStationsSchema, getStationByIdSchema, updateStationSchema, deleteBusStationsSchema, busStationsStatsSchema, createBusStationSchema, suggestBusStationSchema, considerSuggestedStationSchema, autoCompleteBusStationSchema, getInitialBusStationsSchema, bulkUpdateStationSchema } from "./schemas/busStation"
...
router.get("/get_initial_station",
    validateRequest(getInitialBusStationsSchema),

    // verifyPermissions(Object.values(ROLES), Object.values(SUBROLES)),

    tryCatch(Controller.getInitialBusStations))

router.get("/autocomplete",
    validateRequest(autoCompleteBusStationSchema),
    // verifyPermissions((Object.values(ROLES), Object.values(SUBROLES))),
    tryCatch(Controller.autocCompleteBusStations))
...
router.patch("/bulk_update", validateRequest(bulkUpdateStationSchema),

    // verifyPermissions([ROLES.SUPERADMIN, ROLES.ADMIN],[SUBROLES.MANAGER, SUBROLES.STAFF]), 

    tryCatch(Controller.bulkUpdateStations))

router.patch('/update', validateRequest(updateStationSchema),
    verifyPermissions([ROLES.SUPERADMIN, ROLES.ADMIN, ROLES.CX], [SUBROLES.MANAGER, SUBROLES.STAFF]), tryCatch(Controller.updateBusStation))
